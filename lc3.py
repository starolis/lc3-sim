...
class LC3ControlUnit:
    ...
    def execute_add(self, instruction):
        ...
        if imm_flag:
            ...
            self.registers.registers[f"R{dr}"] = (
                self.registers.registers[f"R{sr1}"] + imm5
            ) & 0xFFFF
        else:
            sr2 = instruction & 0x7
            self.registers.registers[f"R{dr}"] = (
                self.registers.registers[f"R{sr1}"]
                + self.registers.registers[f"R{sr2}"]
            ) & 0xFFFF
    ...
    def execute_br(self, instruction):
        ...
        if (
            (n and self.registers.CC["N"])
            or (z and self.registers.CC["Z"])
            or (p and self.registers.CC["P"])
        ):
    ...
    def execute_not(self, instruction):
        ...
        self.registers.registers[f"R{dr}"] = (
            ~self.registers.registers[f"R{sr}"] & 0xFFFF
        )
        self.set_condition_codes(self.registers.registers[f"R{dr}"])
